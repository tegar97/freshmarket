...
class MapProvider extends ChangeNotifier {
  ...
  List<Placemark>? _locationDetails;
  List<Placemark> get locationDetail => _locationDetails!;

  String? _complateAddress;
  String? get complateAddress => _complateAddress;
  ...
  Future<void> initLocation() async {
    ...
    setDetailLocation(_sourceLocation, false);
    setDetailLocation(_sourceLocation, true);
    notifyListeners();
  }
  ...
  void onMapCreated(GoogleMapController controller) async {
    ...
    setDetailLocation(sourceLocation, true);
  ...
  setDetailLocation(LatLng? sourceLocatio, bool isTemp) async {
    _locationDetails = await placemarkFromCoordinates(
        sourceLocatio!.latitude, sourceLocatio!.longitude);
  
    setComplateAddress(_locationDetails, isTemp);

    notifyListeners();
  }

  setComplateAddress(List<Placemark>? location, bool isTemp) {
    if (location != null) {
      if (isTemp == true) {
        _tempAddress =
            '${location[0].street} ${location[0].subLocality}  ${location[0].locality} ${location[0].subAdministrativeArea} ${location[0].administrativeArea} ${location[0].postalCode} ${location[0].country}';
      } else {
        _complateAddress =
            '${location[0].street} ${location[0].subLocality}  ${location[0].locality} ${location[0].subAdministrativeArea} ${location[0].administrativeArea} ${location[0].postalCode} ${location[0].country}';
      }
